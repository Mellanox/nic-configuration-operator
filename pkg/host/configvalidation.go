...
type configValidation interface {
	...
	ConstructNvParamMapFromTemplate(device *v1alpha1.NicDevice, defaultValues map[string]string) (map[string]string, error)
	...
	// returns error - if an error occured during validation
...
func applyDefaultNvConfigValueIfExists(paramName string, desiredParameters map[string]string, defaultValues map[string]string) {
...
// ConstructNvParamMapFromTemplate translates a configuration template into a set of nvconfig parameters
// operates under the assumption that spec validation was already carried out
func (v *configValidationImpl) ConstructNvParamMapFromTemplate(device *v1alpha1.NicDevice, defaultValues map[string]string) (map[string]string, error) {
	...
	if template.LinkType != "" {
		...
		} else if device.Status.Ports[0].NetworkInterface != "" && v.utils.GetLinkType(device.Status.Ports[0].NetworkInterface) != string(device.Spec.Configuration.Template.LinkType) {
			err := types.IncorrectSpecError(fmt.Sprintf("device doesn't support changing link type, wrong link type provided in the template, should be: %s", v.utils.GetLinkType(pciAddr)))
	...
	if template.GpuDirectOptimized != nil && template.GpuDirectOptimized.Enabled {
		...
		if template.PciPerformanceOptimized == nil || !template.PciPerformanceOptimized.Enabled {
			err := types.IncorrectSpecError("GpuDirectOptimized should only be enabled together with PciPerformanceOptimized")
			log.Log.Error(err, "incorrect spec", "device", device.Name)
			return desiredParameters, err
		}
	} else {
...
func (v *configValidationImpl) RuntimeConfigApplied(device *v1alpha1.NicDevice) (bool, error) {
	...
	if desiredMaxReadReqSize != 0 {
		actualMaxReadReqSize, err := v.utils.GetMaxReadRequestSize(ports[0].PCI)
		if err != nil {
		...
		if len(ports) == 2 {
			actualMaxReadReqSize, err = v.utils.GetMaxReadRequestSize(ports[1].PCI)
			if err != nil {
				log.Log.Error(err, "can't validate maxReadReqSize", "device", device.Name)
				return false, err
			}
	...
	actualTrust, actualPfc, err := v.utils.GetTrustAndPFC(ports[0].NetworkInterface)
	if err != nil {
	...
	if len(ports) == 2 {
		actualTrust, actualPfc, err := v.utils.GetTrustAndPFC(ports[1].NetworkInterface)
		if err != nil {
			log.Log.Error(err, "can't validate QoS settings", "device", device.Name)
			return false, err
		}
