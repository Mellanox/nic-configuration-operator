		...
		Describe("ValidateDeviceNvSpec", func() {
			...
			Context("when ResetToDefault is true", func() {
				...
				It("should call ValidateResetToDefault and return its results", func() {
					...
					mockConfigValidation.On("ValidateResetToDefault", nvConfig).Return(true, false, nil)
				...
				It("should return an error if ValidateResetToDefault fails", func() {
					...
					mockHostUtils.On("QueryNvConfig", ctx, pciAddress).Return(nvConfig, nil)
					mockConfigValidation.On("ValidateResetToDefault", nvConfig).Return(false, false, validationErr)
			...
			Context("when ConstructNvParamMapFromTemplate returns an error", func() {
				It("should return false, false, and the error", func() {
					...
					mockHostUtils.On("QueryNvConfig", ctx, pciAddress).Return(nvConfig, nil)
					mockConfigValidation.On("ConstructNvParamMapFromTemplate", device, nvConfig.DefaultConfig).Return(nil, constructErr)
			...
			Context("when desiredConfig fully matches current and next config", func() {
				It("should return false, false, nil", func() {
					...
					mockHostUtils.On("QueryNvConfig", ctx, pciAddress).Return(nvConfig, nil)
					mockConfigValidation.On("ConstructNvParamMapFromTemplate", device, nvConfig.DefaultConfig).Return(desiredConfig, nil)
					mockConfigValidation.On("AdvancedPCISettingsEnabled", nvConfig.CurrentConfig).Return(false)
			...
			Context("when AdvancedPCISettingsEnabled is true and a parameter is missing in CurrentConfig", func() {
				It("should return an IncorrectSpecError", func() {
					...
					mockHostUtils.On("QueryNvConfig", ctx, pciAddress).Return(nvConfig, nil)
					mockConfigValidation.On("ConstructNvParamMapFromTemplate", device, nvConfig.DefaultConfig).Return(desiredConfig, nil)
					mockConfigValidation.On("AdvancedPCISettingsEnabled", nvConfig.CurrentConfig).Return(true)

					expectedErr := types.IncorrectSpecError(fmt.Sprintf("Parameter %s unsupported for device %s", "param1", device.Name))
		...
		Describe("ApplyDeviceNvSpec", func() {
			Context("when ResetToDefault is true", func() {
				...
				It("should reset NV config and set AdvancedPCISettings parameter successfully", func() {
					...
					mockHostUtils.On("SetNvConfigParameter", pciAddress, consts.AdvancedPCISettingsParam, consts.NvParamTrue).Return(nil)
				...
				It("should return error if SetNvConfigParameter fails", func() {
					...
					mockHostUtils.On("SetNvConfigParameter", pciAddress, consts.AdvancedPCISettingsParam, consts.NvParamTrue).Return(setParamErr)
			...
			Context("when ResetToDefault is false", func() {
				...
				Context("when AdvancedPCISettingsEnabled is false", func() {
					It("should set AdvancedPCISettingsParam and reset NIC firmware successfully", func() {
						...
						mockConfigValidation.On("AdvancedPCISettingsEnabled", nvConfig.CurrentConfig).Return(false)
						mockHostUtils.On("SetNvConfigParameter", pciAddress, consts.AdvancedPCISettingsParam, consts.NvParamTrue).Return(nil)
						mockHostUtils.On("ResetNicFirmware", ctx, pciAddress).Return(nil)
						mockHostUtils.On("QueryNvConfig", ctx, pciAddress).Return(nvConfig, nil)
						mockConfigValidation.On("ConstructNvParamMapFromTemplate", device, nvConfig.DefaultConfig).Return(desiredConfig, nil)
					...
					It("should return error if second QueryNvConfig fails", func() {
						...
						mockHostUtils.On("QueryNvConfig", ctx, pciAddress).Return(nvConfig, nil).Times(1)
						mockConfigValidation.On("AdvancedPCISettingsEnabled", nvConfig.CurrentConfig).Return(false)
						mockHostUtils.On("SetNvConfigParameter", pciAddress, consts.AdvancedPCISettingsParam, consts.NvParamTrue).Return(nil)
						mockHostUtils.On("ResetNicFirmware", ctx, pciAddress).Return(nil)
						secondQueryErr := errors.New("failed to query nv config again")
						mockHostUtils.On("QueryNvConfig", ctx, pciAddress).Return(types.NewNvConfigQuery(), secondQueryErr)
				...
				Context("when AdvancedPCISettingsEnabled is true", func() {
					...
					It("should construct desiredConfig and apply necessary changes successfully", func() {
						...
						mockConfigValidation.On("AdvancedPCISettingsEnabled", nvConfig.CurrentConfig).Return(true)
						mockConfigValidation.On("ConstructNvParamMapFromTemplate", device, nvConfig.DefaultConfig).Return(desiredConfig, nil)
						mockHostUtils.On("SetNvConfigParameter", pciAddress, "param1", "value2").Return(nil)
					...
					It("should return error if desiredConfig has a parameter not in NextBootConfig", func() {
						...
						expectedErr := types.IncorrectSpecError(fmt.Sprintf("Parameter %s unsupported for device %s", "param2", device.Name))
					...
					It("should return error if SetNvConfigParameter fails while applying params", func() {
						...
						desiredConfig := map[string]string{"param1": "value3", "param2": "value3"}

						mockHostUtils.On("QueryNvConfig", ctx, pciAddress).Return(nvConfig, nil)
						mockConfigValidation.On("AdvancedPCISettingsEnabled", nvConfig.CurrentConfig).Return(true)
						mockConfigValidation.On("ConstructNvParamMapFromTemplate", device, nvConfig.DefaultConfig).Return(desiredConfig, nil)
						mockHostUtils.On("SetNvConfigParameter", pciAddress, "param1", "value3").Return(nil)
						setParamErr := errors.New("failed to set param2")
						mockHostUtils.On("SetNvConfigParameter", pciAddress, "param2", "value3").Return(setParamErr)
			...
			Context("when applying multiple parameters", func() {
				It("should apply all parameters successfully and require a reboot", func() {
					...
					mockConfigValidation.On("AdvancedPCISettingsEnabled", nvConfig.CurrentConfig).Return(true)
					mockConfigValidation.On("ConstructNvParamMapFromTemplate", device, nvConfig.DefaultConfig).Return(desiredConfig, nil)
					mockHostUtils.On("SetNvConfigParameter", pciAddress, "param1", "newValue3").Return(nil)
					mockHostUtils.On("SetNvConfigParameter", pciAddress, "param2", "newValue3").Return(nil)
