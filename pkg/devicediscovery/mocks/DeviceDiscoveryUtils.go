...
func (_m *DeviceDiscoveryUtils) GetNetworkBayASIC(pciAddr string) (int, bool) {
	...
	if len(ret) == 0 {
		panic("no return value specified for GetNetworkBayASIC")
	}
...
// GetVPD provides a mock function with given fields: pciAddr
func (_m *DeviceDiscoveryUtils) GetVPD(pciAddr string) (*types.VPD, error) {
	ret := _m.Called(pciAddr)

	if len(ret) == 0 {
		panic("no return value specified for GetVPD")
	}

	var r0 *types.VPD
	var r1 error
	if rf, ok := ret.Get(0).(func(string) (*types.VPD, error)); ok {
		return rf(pciAddr)
	}
	if rf, ok := ret.Get(0).(func(string) *types.VPD); ok {
		r0 = rf(pciAddr)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*types.VPD)
		}
	}
