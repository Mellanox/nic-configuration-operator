...
func (m *udevManager) reloadUdevRules(ctx context.Context) error {
	...
	// Reload udev rules
	cmd := m.execInterface.CommandContext(ctx, "udevadm", "control", "--reload-rules")
	...
	cmd = m.execInterface.CommandContext(ctx, "udevadm", "trigger", "--action=add", "--subsystem-match=infiniband")
	if _, err := utils.RunCommand(cmd); err != nil {
	...
	cmd = m.execInterface.CommandContext(ctx, "udevadm", "trigger", "--action=add", "--subsystem-match=net")
	if _, err := utils.RunCommand(cmd); err != nil {
	...
	cmd = m.execInterface.CommandContext(ctx, "udevadm", "settle")
	if _, err := utils.RunCommand(cmd); err != nil {
...
func (m *udevManager) generateUdevRules(devices []*v1alpha1.NicDevice) (netRules, rdmaRules string, expectedNames map[string]ExpectedInterfaceNames) {
	...
	for _, device := range devices {
		...
		for portIndex, port := range device.Status.Ports {
			...
			if netDeviceName != "" {
				netBuilder.WriteString(generateNetDeviceRule(port.PCI, netDeviceName))
			...
			if rdmaDeviceName != "" {
				rdmaBuilder.WriteString(generateRdmaDeviceRule(port.PCI, rdmaDeviceName))
...
func generateRdmaDeviceRule(pciAddress, deviceName string) string {
	// Format: ACTION=="add", KERNELS=="<pci>", SUBSYSTEM=="infiniband", PROGRAM="rdma_rename %k NAME_FIXED <name>"
	return fmt.Sprintf(`ACTION=="add", KERNELS=="%s", SUBSYSTEM=="infiniband", PROGRAM="rdma_rename %%k NAME_FIXED %s"`, pciAddress, deviceName)
}
