...
var _ = Describe("UdevManager", func() {
	...
	Describe("ApplyUdevRules", func() {
		It("should create separate udev rules files for net and RDMA devices and reload udev", func() {
			...
			Expect(rdmaContentStr).To(ContainSubstring(`ACTION=="add", KERNELS=="0000:1a:00.0", SUBSYSTEM=="infiniband", PROGRAM="rdma_rename %k NAME_FIXED rdma1p1"`))
			Expect(rdmaContentStr).To(ContainSubstring(`ACTION=="add", KERNELS=="0000:1a:00.1", SUBSYSTEM=="infiniband", PROGRAM="rdma_rename %k NAME_FIXED rdma1p2"`))
	...
	Describe("generateRdmaDeviceRule", func() {
		It("should generate correct rdma device rule", func() {
			...
			Expect(rule).To(Equal(`ACTION=="add", KERNELS=="0000:1a:00.0", SUBSYSTEM=="infiniband", PROGRAM="rdma_rename %k NAME_FIXED rdma_nic1"`))
		})
	})
})
