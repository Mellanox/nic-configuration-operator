...
func (r *NicDeviceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if configStatuses.rebootRequired() {
		requeue, err := r.handleReboot(ctx, configStatuses)
		if err != nil {
			return ctrl.Result{}, err
		}
		if requeue {
	...
	if configStatuses.rebootRequired() {
		...
		// Returning and waiting for reboot
		return ctrl.Result{}, nil
	}
...
func (r *NicDeviceReconciler) applyRuntimeConfig(ctx context.Context, statuses nicDeviceConfigurationStatuses) error {
	...
	for i := 0; i < len(statuses); i++ {
		...
		go func(index int) {
			...
			err := r.hostManager.ApplyDeviceRuntimeSpec(statuses[i].device)
			if err != nil {
				statuses[i].lastStageError = err
...
func (r *NicDeviceReconciler) handleReboot(ctx context.Context, statuses nicDeviceConfigurationStatuses) (bool, error) {
	err := r.maintenanceManager.ScheduleMaintenance(ctx)
	if err != nil {
		log.Log.Error(err, "failed to schedule maintenance for node")
		return true, err
...
func (r *NicDeviceReconciler) applyNvConfig(ctx context.Context, statuses nicDeviceConfigurationStatuses) error {
	...
	for i := 0; i < len(statuses); i++ {
		...
		go func(index int) {
			...
			rebootRequired, err := r.hostManager.ApplyDeviceNvSpec(ctx, statuses[i].device)
			if err != nil {
				statuses[i].lastStageError = err
			...
			statuses[i].rebootRequired = rebootRequired
		}(i)
	}
...
func NewNicDeviceReconciler(client client.Client, scheme *runtime.Scheme, nodeName string, namespaceName string, hostManager host.HostManager, maintenanceManager maintenance.MaintenanceManager) *NicDeviceReconciler {
	return &NicDeviceReconciler{
		Client:             client,
		Scheme:             scheme,
		nodeName:           nodeName,
