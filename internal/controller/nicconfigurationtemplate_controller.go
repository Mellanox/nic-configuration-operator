	...
	for _, device := range deviceList.Items {
		...
		for _, template := range templates {
			if !deviceMatchesSelectors(&device, template, node) {
				err := r.dropDeviceFromStatus(ctx, device.Name, template)
				if err != nil {
					log.Log.Error(err, "Failed to drop device name from template status", "template", template)
				}
		...
		if len(matchingTemplates) == 0 {
			...
			return ctrl.Result{}, r.handleErrorSeveralMatchingTemplates(ctx, &device, joinedTemplateNames)
		}
...
func (r *NicConfigurationTemplateReconciler) dropDeviceFromStatus(ctx context.Context, deviceName string, template *v1alpha1.NicConfigurationTemplate) error {
	if slices.Contains(template.Status.NicDevices, deviceName) {
