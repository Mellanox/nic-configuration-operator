...
func (r *NicFirmwareSourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if instance.Spec.BFBUrlSource != "" {
		bfbFileName, err = r.processBFBSource(ctx, instance, cacheName)
		if err != nil {
	...
	if err = r.updateStatus(ctx, instance, consts.FirmwareSourceSuccessStatus, nil, binaryVersions, bfbFileName); err != nil {
		return reconcile.Result{}, err
	}
...
func (r *NicFirmwareSourceReconciler) processBFBSource(ctx context.Context, instance *v1alpha1.NicFirmwareSource, cacheName string) (string, error) {
	...
	if needsDownload {
		...
		log.Log.Info("BFB file already present, skipping download", "cacheName", instance.Name)
		// We still need to determine the filename for status update
		// This is a simplified approach - in a real implementation, you might want to
		// extract this from the metadata or return it from VerifyCachedBFB
		bfbFileName = filepath.Base(instance.Spec.BFBUrlSource)
	}
...
func (r *NicFirmwareSourceReconciler) updateStatus(ctx context.Context, obj *v1alpha1.NicFirmwareSource, status string, statusError error, versions map[string][]string, bfb string) error {
